...
select nombre, apellido1, apellido2, email from empleado e 
where codigo_jefe = 7
...
select nombre, apellido1, apellido2, puesto from empleado e 
...
select distinct codigo_cliente from pago p 
...
select distinct codigo_cliente from pago p 
where fecha_pago < '2009-01-01' and fecha_pago > '2007-12-31'
...
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido p 
...
where estado like "rechazado" and YEAR(fecha_entrega) = 2009
...
select c.nombre_cliente, c.codigo_empleado_rep_ventas, e.nombre, e.apellido1, e.apellido2 from cliente c 
